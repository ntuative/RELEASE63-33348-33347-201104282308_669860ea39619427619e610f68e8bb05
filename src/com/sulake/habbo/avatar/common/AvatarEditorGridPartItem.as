package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_634:Rectangle;
      
      private var var_1880:int = 0;
      
      private var var_436:IAvatarRenderManager;
      
      private var var_1082:Boolean;
      
      private var var_1881:BitmapData;
      
      private const const_2148:int = 13421772;
      
      private const const_2147:int = 16777215;
      
      private const const_366:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_703 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1880 = Math.max(this.var_1880,_loc6_.colorLayerIndex);
            }
         }
         this.var_436 = this.var_67.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_703 != null && !this.var_1882)
            {
               _loc3_ = this.var_703;
            }
            else
            ...
            if(this.var_785)
            {
               switch(this.var_785.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_43:
                     _loc2_.style = HabboIconType.const_851;
                     break;
         ...
         if(this.isSelected)
         {
            this.var_1487.color = this.const_2148;
         }
         else
         {
            this.var_1487.color = this.const_2147;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_436.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_436.isFigureReady(_loc1_))
         {
            this.var_436.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_366[_loc6_] + "_" + FigureData.const_158;
               _loc4_ = this.var_436.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_366.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_366[_loc6_] + "_" + FigureData.const_158;
                  _loc4_ = this.var_436.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_634 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1881)
               {
                  ...
                  this.var_1881 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_634.width,this.var_634.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_634.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_634.y;
               if(this.var_1882 && _loc2_.colorLayerIndex > 0)
               {
