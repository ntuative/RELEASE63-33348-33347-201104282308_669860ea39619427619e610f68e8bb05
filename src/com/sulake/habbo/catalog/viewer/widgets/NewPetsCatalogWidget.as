package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class NewPetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IGetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_527 = this.getPetTypeIndexFromProduct(_loc2_.localizationId);
         if(this.var_527 < 8)
         {
            return false;
         }
         this.var_2634 = _loc2_.localizationId;
         this.updateAvailableBreeds(_loc2_.localizationId);
         if(this.var_203 != null && this.var_203.length > 0)
         {
            this.var_374 = this.var_203[0];
         }
         else
         {
            this.var_374 = -1;
         }
         this._offers.add(this.var_527,_loc2_);
      ...
      private function initializeBreedSelection() : void
      {
         ...
         if(this.var_203 != null)
         {
            _loc1_ = [];
            _loc2_ = 0;
            while(_loc2_ < const_1427 && _loc2_ < this.var_203.length)
            {
               _loc3_ = (page.viewer.catalog as HabboCatalog).roomEngine.getPetColor(this.var_527,this.var_203[_loc2_]);
               if(_loc3_ != null)
               {
      ...
      private function selectedBreed(param1:int) : void
      {
         if(this.var_203 != null && this.var_203.length > 0)
         {
            ...
            this.var_374 = this.var_203[param1];
            this.updateImage();
         }
      }
      ...
      private function getPetLocalization() : String
      {
         ...
         if(page != null && false)
         {
            ...
            {
               return _loc1_.localization.getKey(this.getRaceLocalizationKey(this.var_527,this.var_374),this.getRaceLocalizationKey(this.var_527,this.var_374));
            }
         }
         return this.getRaceLocalizationKey(this.var_527,this.var_374);
      }
