package com.sulake.habbo.quest
{
   ...
   public class QuestCompleted implements IDisposable
   {
      ...
      private function onNextQuest(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this._window.visible = false;
         if(this.var_347.completedQuestsInCampaign >= this.var_347.questCountInCampaign)
         {
            this.var_347.id = 0;
            this.var_347.accepted = false;
            this._questEngine.controller.campaignCompleted.show(this.var_347);
         }
         else
         {
      ...
      public function prepare(param1:QuestMessageData) : void
      {
         ...
         this._window.findChildByName("catalog_link_txt").caption = this._questEngine.localization.getKey("quests.completed.cataloglink." + this.var_347.activityPointType);
         var _loc2_:String = "quests.completed.reward." + this.var_347.activityPointType;
         this._questEngine.localization.registerParameter(_loc2_,"amount",this.var_347.rewardCurrencyAmount.toString());
         this._window.findChildByName("reward_txt").caption = this._questEngine.localization.getKey(_loc2_,_loc2_);
         var _loc3_:* = this.var_347.getQuestLocalizationKey() + ".completed";
