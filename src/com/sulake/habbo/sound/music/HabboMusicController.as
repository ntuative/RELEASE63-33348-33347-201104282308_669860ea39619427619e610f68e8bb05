package com.sulake.habbo.sound.music
{
   ...
   public class HabboMusicController implements IHabboMusicController, IDisposable
   {
      ...
      private static const const_1417:int = HabboMusicPrioritiesEnum.const_121;
      ...
      private var var_517:IPlayListController = null;
      ...
      public function HabboMusicController(param1:HabboSoundManagerFlash10, param2:IEventDispatcher, param3:IEventDispatcher, param4:IConnection)
      {
         ...
         this.var_736.push(new TraxSongInfoMessageEvent(this.onSongInfoMessage));
         this.var_736.push(new UserSongDisksInventoryMessageEvent(this.onSongDiskInventoryMessage));
         for each(_loc5_ in this.var_736)
         {
            this._connection.addMessageEvent(_loc5_);
         }
         this.var_79.addEventListener(RoomEngineSoundMachineEvent.const_972,this.onJukeboxInit);
         this.var_79.addEventListener(RoomEngineSoundMachineEvent.const_247,this.onJukeboxDispose);
         this.var_79.addEventListener(RoomEngineSoundMachineEvent.const_924,this.onSoundMachineInit);
         this.var_79.addEventListener(RoomEngineSoundMachineEvent.const_699,this.onSoundMachineDispose);
         this.var_1213 = new Timer(1000);
         this.var_1213.start();
         this.var_1213.addEventListener(TimerEvent.TIMER,this.sendNextSongRequestMessage);
         ...
         {
            this.var_518[_loc6_] = null;
            this.var_791[_loc6_] = 0;
            _loc6_++;
         }
      }
      ...
      protected function onSongFinishedPlayingEvent(param1:SoundCompleteEvent) : void
      {
         ...
         {
            if(this.getTopRequestPriority() == this._priorityPlaying && this.getSongRequestCountAtPriority(this._priorityPlaying) == this.var_1900)
            {
               this.resetSongStartRequest(this._priorityPlaying);
            }
            _loc2_ = this._priorityPlaying;
            this.playSongWithHighestPriority();
            if(_loc2_ >= HabboMusicPrioritiesEnum.const_405)
            {
               this._events.dispatchEvent(new NowPlayingEvent(NowPlayingEvent.const_120,_loc2_,param1.id,-1));
            }
         }
      }
      ...
      public function dispose() : void
      {
         ...
         if(!this._disposed)
         {
            ...
            if(this.var_79)
            {
               this.var_79.removeEventListener(RoomEngineSoundMachineEvent.const_972,this.onJukeboxInit);
               this.var_79.removeEventListener(RoomEngineSoundMachineEvent.const_247,this.onJukeboxDispose);
               this.var_79.removeEventListener(RoomEngineSoundMachineEvent.const_924,this.onSoundMachineInit);
               this.var_79.removeEventListener(RoomEngineSoundMachineEvent.const_699,this.onSoundMachineDispose);
            }
      ...
      private function addSongStartRequest(param1:int, param2:int, param3:Number, param4:Number, param5:Number, param6:Number) : Boolean
      {
         ...
         this.var_518[param1] = _loc7_;
         this.var_791[param1] += 1;
         return true;
      }
      ...
      private function getSongIdRequestedAtPriority(param1:int) : int
      {
         ...
         if(this.var_518[param1] == null)
         {
            return -1;
         }
         var _loc2_:SongStartRequestData = this.var_518[param1];
      ...
      {
         var _loc1_:int = this.var_518.length - 1;
         while(_loc1_ >= 0)
         {
            if(this.var_518[_loc1_] != null)
            {
               return _loc1_;
            }
            _loc1_--;
      ...
      {
         if(this.var_640.getValue(param1) == null)
         {
            this.var_640.add(param1,param2);
      ...
      {
         if(param1 >= 0 && param1 < this.var_370.length)
         {
            return this.var_370.getKey(param1);
         }
         return -1;
      }
      ...
      public function getSongDiskInventorySongId(param1:int) : int
      {
         if(param1 >= 0 && param1 < this.var_370.length)
         {
            return this.var_370.getWithIndex(param1);
         }
         return -1;
      }
      ...
      private function onSongInfoMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc5_ < _loc4_.length)
         {
            ...
            if(_loc7_)
            {
               ...
               this._events.dispatchEvent(new SongInfoReceivedEvent(SongInfoReceivedEvent.const_101,_loc6_.id));
               while(this.var_792.indexOf(_loc6_.id) != -1)
               {
                  this.var_792.splice(this.var_792.indexOf(_loc6_.id),1);
      ...
      private function playSongObject(param1:int, param2:int) : Boolean
      {
         ...
         this.var_1900 = this.getSongRequestCountAtPriority(param1);
         this.var_1901 = param2;
         if(this._priorityPlaying <= const_1417)
         {
      ...
      private function notifySongPlaying(param1:SongDataEntry) : void
      {
         ...
         if(param1.length >= 8000 && (this.var_2627 != param1.id || _loc3_ > this.var_2626 + 8000))
         {
            this._soundManager.notifyPlayedSong(param1.name,param1.creator);
            this.var_2627 = param1.id;
            this.var_2626 = _loc3_;
         }
      }
      ...
      private function onSongDiskInventoryMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc4_ < _loc3_.songDiskCount)
         {
            ...
            this.var_370.add(_loc5_,_loc6_);
            if(this.var_439.getValue(_loc6_) == null)
            {
               this.var_792.push(_loc6_);
      ...
      private function onSoundMachineInit(param1:Event) : void
      {
         ...
         this.var_517 = new SoundMachinePlayListController(this._soundManager,this,this._events,this.var_79,this._connection) as IPlayListController;
      }
      ...
      {
         this.disposeRoomPlaylist();
         this.var_517 = new JukeboxPlayListController(this._soundManager,this,this._events,this._connection) as IPlayListController;
