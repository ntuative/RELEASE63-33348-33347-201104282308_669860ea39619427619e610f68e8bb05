package com.sulake.habbo.tracking
{
   ...
   public class LatencyTracker
   {
      ...
      public function init() : void
      {
         ...
         this.var_1678 = int(this.var_192.getKey("latencytest.interval"));
         this.var_1679 = int(this.var_192.getKey("latencytest.report.index"));
         this.var_2209 = int(this.var_192.getKey("latencytest.report.delta"));
      ...
      public function update(param1:uint, param2:int) : void
      {
         ...
         if(param2 - this.var_1676 > this.var_1678)
         {
            this.testLatency();
         }
      }
      ...
      private function testLatency() : void
      {
         ...
         this.var_591.add(this.var_1376,this.var_1676);
         this._connection.send(new LatencyPingRequestMessageComposer(this.var_1376));
         ++this.var_1376;
      }
      ...
      private function onPingResponse(param1:IMessageEvent) : void
      {
         ...
         var _loc10_:int = 0;
         var _loc11_:* = null;
         if(this.var_591 == null || this.var_218 == null)
         {
            return;
         }
         var _loc2_:LatencyPingResponseMessageParser = (param1 as LatencyPingResponseMessageEvent).getParser();
         var _loc3_:int = this.var_591.getValue(_loc2_.requestId);
         this.var_591.remove(_loc2_.requestId);
         var _loc4_:int = getTimer() - _loc3_;
         this.var_218.push(_loc4_);
         if(this.var_218.length == this.var_1679 && this.var_1679 > 0)
         {
            _loc5_ = 0;
            _loc6_ = 0;
            _loc7_ = 0;
            ...
            if(Math.abs(_loc9_ - this.var_1677) > this.var_2209 || this.var_1677 == 0)
            {
               this.var_1677 = _loc9_;
